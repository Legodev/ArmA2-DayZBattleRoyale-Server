...
if (DZEBR_EnableBattleroyal) then {
	if(isnil "DZEBR_GlobalNextCharid") then {
		DZEBR_GlobalNextCharid = 10;
	};

	_primary = [true, DZEBR_GlobalNextCharid, "Survivor2_DZ", dayz_hiveVersionNo];

	DZEBR_GlobalNextCharid = DZEBR_GlobalNextCharid + 1;
} else {
