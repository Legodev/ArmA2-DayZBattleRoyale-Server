...
if (DZEBR_EnableBattleroyal) then {
	server_hiveWrite = {
		diag_log ("BLOCKED WRITE: " + _this);
	};
	...
	call compile preProcessFileLineNumbers format ["\z\addons\dayz_server\battleroyal\buildings\%1_buildings.sqf",_map];
	call compile preProcessFileLineNumbers format ["\z\addons\dayz_server\battleroyal\buildings\%1_trees.sqf",_map];

	//spawn vehicles
	//[] execVM "\z\addons\dayz_server\compile\server_spawnVehicles.sqf";
	call compile preProcessFileLineNumbers "\z\addons\dayz_server\battleroyal\vehicles\spawnVehicles.sqf";
